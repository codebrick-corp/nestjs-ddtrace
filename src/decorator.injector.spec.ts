...
describe('DecoratorInjector', () => {
  ...
  it('should exclude provider and all its methods', async () => {
    ...
    expect(result1).toBe(0);
    expect(result2).toBe(1);
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloService.prototype.hi)).toBeUndefined();
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloService.prototype.hello)).toBeUndefined();
  ...
  it('should exclude methods in provider', async () => {
    ...
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloService.prototype.hello)).toBeUndefined();
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloService.hi', { childOf: null });
    expect(tracer.startSpan).not.toHaveBeenCalledWith('HelloService.hello', { childOf: null });
  ...
  it('should exclude controller and all its methods', async () => {
    ...
    // then
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloController.prototype.hi)).toBeUndefined();
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloController.prototype.hello)).toBeUndefined();
    expect(Reflect.getMetadata(PATH_METADATA, HelloController.prototype.hi)).toBe('/hi');
    expect(Reflect.getMetadata(PATH_METADATA, HelloController.prototype.hello)).toBe('/hello');
  ...
  it('should exclude methods in controller', async () => {
    ...
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloController.hi', { childOf: null });
    expect(tracer.startSpan).not.toHaveBeenCalledWith('HelloController.hello', { childOf: null });
    expect(tracer.scope().active).toHaveBeenCalledTimes(1);
    expect(tracer.scope().activate).toHaveBeenCalledTimes(1);
    expect(mockSpan.finish).toHaveBeenCalledTimes(1);
  ...
  it('should exclude controller if included in options.excludeControllers', async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      imports: [DatadogTraceModule.forRoot({ controllers: true, excludeControllers: ['WorldController'] })],
      controllers: [HelloController, WorldController]
    }).compile();
    const app = module.createNestApplication();
    await app.init();
    ...
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloController.prototype.hi)).toBe(1);
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloController.prototype.hello)).toBe(1);
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, WorldController.prototype.bar)).toBeUndefined();
    expect(Reflect.getMetadata(PATH_METADATA, HelloController.prototype.hi)).toBe('/hi');
    expect(Reflect.getMetadata(PATH_METADATA, HelloController.prototype.hello)).toBe('/hello');
    expect(Reflect.getMetadata(PATH_METADATA, WorldController.prototype.bar)).toBe('/bar');
    ...
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloController.hi', { childOf: null });
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloController.hello', { childOf: null });
    expect(tracer.startSpan).not.toHaveBeenCalledWith('WorldController.bar', { childOf: null });
    expect(tracer.scope().active).toHaveBeenCalledTimes(2);
    expect(tracer.scope().activate).toHaveBeenCalledTimes(2);
    expect(mockSpan.finish).toHaveBeenCalledTimes(2);
  ...
  it('should exclude provider if included in options.excludeProviders', async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      imports: [DatadogTraceModule.forRoot({ providers: true, excludeProviders: ['WorldService'] })],
      providers: [HelloService, WorldService],
    }).compile();
    ...
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloService.prototype.hi)).toBe(1);
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, HelloService.prototype.hello)).toBe(1);
    expect(Reflect.getMetadata(Constants.SPAN_METADATA_ACTIVE, WorldService.prototype.foo)).toBeUndefined();
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloService.hi', { childOf: null });
    expect(tracer.startSpan).toHaveBeenCalledWith('HelloService.hello', { childOf: null });
    expect(tracer.startSpan).not.toHaveBeenCalledWith('WorldService.foo', { childOf: null });
    expect(tracer.scope().active).toHaveBeenCalledTimes(2);
    expect(tracer.scope().activate).toHaveBeenCalledTimes(2);
    expect(mockSpan.finish).toHaveBeenCalledTimes(2);
